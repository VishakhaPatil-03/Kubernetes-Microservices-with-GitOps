pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'vishupatil14/techsolutions-app'
        K8S_CLUSTER_NAME = 'Micro-project'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/VishakhaPatil-03/Kubernetes-Microservices-with-GitOps.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|vishupatil14/techsolutions-app:latest|vishupatil14/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
